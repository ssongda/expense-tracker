...
export const ExpenseInput = ({ selectedDate, refetchExpenses }: Props) => {
  const { categoryRef, amountRef, handleAddExpense, handleInputChange } =
    useExpenseInput({
      selectedDate,
      refetchExpenses,
    });
  ...
  return (
    <form className={styles.inputContainer} onSubmit={handleAddExpense}>
      <select ref={categoryRef} defaultValue={DEFAULT_EXPENSE_TYPES[0]}>
